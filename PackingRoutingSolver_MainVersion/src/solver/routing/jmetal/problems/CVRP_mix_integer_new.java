 ...
 * instances of TSPLIB
 */
public class CVRP_mix_integer_new extends Problem {
  ...
  static int encoding_strategy;  // 1 = full encoding (s,x,p); 2 = encoding without split encoding (simply use half) (x,p); 
  ...
  public static double threshold;
	...
	*/
  public CVRP_mix_integer_new(
		 int encoding_strategy0,
		  		String solutionType,
              double split_minv,
...
//    truck_weight_ratio = truck_weight_ratio0;
    ...
    encoding_strategy = encoding_strategy0;
...
//    length_       = new int[numberOfVariables_];
    ...
    for (int i = 0; i < NUM_nodes; i++) {
     ...
     if (solutionType.compareTo("PermutationBinary") == 0)
         if (encoding_strategy == 1) {
         ...
         else if (encoding_strategy == 2 || encoding_strategy == 3  ) {
             numberOfVariables_ = 2;//è‡³å°‘ä¸€å€‹ç¯€é»žã€‚
	         length_       = new int[numberOfVariables_];
	         length_      [0] = new_num_nodes;
	         length_      [1] = new_num_nodes-1;
        	 ...
        	 solutionType_ = new PermutationBinarySolutionType(this,1,1) ;
         }
         else if (encoding_strategy == 4) {
             numberOfVariables_ = 1;//è‡³å°‘ä¸€å€‹ç¯€é»žã€‚
	         length_       = new int[numberOfVariables_];
	         length_      [0] = new_num_nodes;

	         numberOfCities_ = new_num_nodes;
	         solutionType_ = new PermutationBinarySolutionType(this,1,0) ;
         }
...
//    	} 
    ...
    if (encoding_strategy == 1 ) {
        ...
        for (int i = 0; i < NUM_nodes; i++) {
        	if  (nodes_v_w[i][0] > Split_minv) {//æœ‰æ–°å¢žçš„ç¯€é»ž
        	    new_node_no[new_num_nodes] = i+1;//æ–°å¢žä¸€å€‹è§£é»žï¼Œ
            	new_node_v[new_num_nodes] = nodes_v_w[i][0]*0.5;//æ–°å¢žç¯€é»žå°�æ‡‰çš„é«�?ç©�ã€‚
            	new_num_nodes += 1; 
        	    //new_node_coords[new_num_nodes] = node_coords[i];
            	new_node_no[new_num_nodes] = i+1;
            	new_node_v[new_num_nodes] = nodes_v_w[i][0]*0.5;//6.0
            	new_num_nodes += 1; 
    ...
    if (encoding_strategy == 1 || encoding_strategy == 2 || encoding_strategy == 3) {
        ...
        for (int i = 0; i < (numberOfCities_ ); i++) {
            ...
            if ( if_large_box[new_node_no[x]-1]==1 || encoding_strategy == 3) {
...
//        	  }
        ...
        for (int i = 0; i < (numberOfCities_ ); i++) {
          ...
          if (i==0) {
              ...
              x_last = ((Permutation)solution.getDecisionVariables()[0]).vector_[i-1] ; //ä¸Šä¸€å€‹ç¯€é»žã€‚
              //System.out.println(if_start0[i-1]);
              if (weights_precent >= (VEHICLE_VOLUME[Type_trucks-1]*Relax_ratio)) {//æ˜¯å�¦é–‹å§‹ç¯€é»žã€‚
            	  fitness1 += distance2_start_end(NUM_nodes+1,x_last);
            	  fitness1 += distance2_start_end(0,x);
...
//			        solution.evaluation();
		...
		if (encoding_strategy == 1 || encoding_strategy == 2 || encoding_strategy == 3) {
			...
			n_route = 0; // number of customers each route has
...
//		        		Box b = clients_trans.get(node_id).getGoods().get(k);
			...
			if (unloadboxes.size() >0) {
				...
				if (encoding_strategy == 3 || encoding_strategy == 4) {
					truck_start = NUM_trucks - 1;
				}

		        for (int j = truck_start; j < NUM_trucks; j++) {
