...
package solver.routing.jmetal.operators.mutation;
...
import solver.routing.jmetal.encodings.solutionType.PermutationSolutionType;
...
import solver.routing.jmetal.util.wrapper.XReal;
		    ...
		    if (solution.getType().getClass() == PermutationBinarySolutionType.class) {
		      ...
		      if (PseudoRandom.randDouble() < SwapProbability) {
				...
				for (int i = pos_route; i < permutationLength-1; i++ ) {
					...
					up = permutationLength-1;
				}
				
		        if (low!=up-1) {
			        pos1 = PseudoRandom.randInt(low,up) ;
//			        for (int nn =0;nn<50;nn++) {
//			        	 System.out.println(PseudoRandom.randInt(low,up));
//			        }
//			        int low0 = pos1-low;
//			        int up0 = up-1-low;
			        pos2 = pos1;
			        for (int nn=0;nn<5;nn++) {
				        pos2 = PseudoRandom.randInt(low,up) ;
				        if (pos2 != pos1) {
				        	break;
				        }
			        }
