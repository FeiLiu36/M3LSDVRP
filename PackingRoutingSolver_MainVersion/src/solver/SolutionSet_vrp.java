...
public class SolutionSet_vrp implements Serializable {
	public final List<Solution_vrp> solutionList_;
	  /** 
...
//		}
	...
	public double[] getIdealNadir() {
		double[] out = new double[4];//the normalized objective values
		out[0] = 1e20; 
		out[1] = 1e20;
		out[2] = 0;
		out[3] = 0;
		...
		for(int i=0;i<solutionList_.size();i++) {
			if (solutionList_.get(i).getF1()<out[0]) {
				out[0] = solutionList_.get(i).getF1();		
			}
			if (solutionList_.get(i).getF2()<out[1]) {
				out[1] = solutionList_.get(i).getF2();		
			}
			if (solutionList_.get(i).getF1()>out[2]) {
				out[2] = solutionList_.get(i).getF1();		
			}
			if (solutionList_.get(i).getF2()>out[3]) {
				out[3] = solutionList_.get(i).getF2();		
			}
	...
	/** 
	   ...
	   */
	  public void printObjectivesToFile(String path1,String path2, Double[] idealNadir,int cn, int bn, double hv){
	    try {
			double minF1 = idealNadir[0],minF2 = idealNadir[1],maxF1=idealNadir[2],maxF2=idealNadir[3];
	      ...
	      for (int i = 0; i < f.length; i++) {
	        ...
	        bw.write(Double.toString(f[i][0])+"  ");
	        bw.write(Double.toString(f[i][1]));
	        bw.newLine();
	        //}
	      }
	      ...
	      for (int i = 0; i < f.length; i++) {
	        //if (this.vector[i].getFitness()<1.0) {
	        bw2.write(Double.toString(solutionList_.get(i).getF1())+"  ");
	        bw2.write(Double.toString(solutionList_.get(i).getF2()));
	        bw2.newLine();
	        //}
	      }
...
//					}
	   ...
	   * @param path The output file name
	   */
	  public void writeResultsFinal(String path, Double[] idealNadir, double timeCost, double hv, int nPacking, double timePacking){
	    try {
			double minF1 = idealNadir[0],minF2 = idealNadir[1],maxF1=idealNadir[2],maxF2=idealNadir[3];
